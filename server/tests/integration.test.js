...
async function runFullStackTests() {
  ...
  try {
    ...
    const msgRemaining = testData.messages.length > 0
      ? await repos.message.findById(testData.messages[0])
      : null;
    ...
    const userRemaining = testData.users.length > 0
      ? await repos.user.findById(testData.users[0])
      : null;
