...
Two parallel jobs triggered on `push` to `main`, `pull_request` to `main`, or manual `workflow_dispatch`:
...
|----------|-------|-------|
| `MONGODB_URL` | Server job env | `mongodb://localhost:27017/test_chatapp_db` (service container) |
| `JWT_SECRET` | Server job env | CI-only test secret |
| `REACT_APP_API_URL` | Web job env | GitHub repository variable (`vars.REACT_APP_API_URL`) |

### Deployment Branches

- **`server`** — orphan branch with production server code only (no tests, no devDependencies)
- **`web`** — orphan branch with built React app (`web/build/` contents)
- Deploy commits use `[skip ci]` suffix to prevent recursive workflow triggers
...
Before pushing workflow changes, validate locally to catch errors without CI round-trips:
