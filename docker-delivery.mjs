 ...
 *
 * Usage examples:
 *   node docker-delivery.mjs --targets server,web --mode build-only --web-api-url https://example.com
 *   node docker-delivery.mjs --targets mobile --mode build-only --mobile-api-url http://localhost:5000
...
const DEFAULT_TRIVY_IMAGE = 'aquasecurity/trivy:0.69.3';
...
// Build args
const webApiUrl = args['web-api-url'] || process.env.WEB_API_URL || process.env.REACT_APP_API_URL || 'http://localhost:5000';
const publicUrl = args['public-url'] || process.env.PUBLIC_URL || '/';
const mobileApiUrl = args['mobile-api-url'] || process.env.MOBILE_API_URL || process.env.API_URL || 'http://localhost:5000';
...
const trivySeverities = args['trivy-severities'] || process.env.TRIVY_SEVERITIES || 'CRITICAL,HIGH,MEDIUM';
const trivyVulnType = args['trivy-vuln-type'] || process.env.TRIVY_VULN_TYPE || 'os,library';
const trivyFormat = args['trivy-format'] || process.env.TRIVY_FORMAT || 'table';
const trivyExitCode = Number(args['trivy-exit-code'] || process.env.TRIVY_EXIT_CODE || 1);
...
function validate(targetsToValidate) {
  ...
  if (targetsToValidate.includes('server') || targetsToValidate.includes('web')) {
    run('node', [path.join(ROOT, 'check-docker-config.mjs')], { cwd: ROOT });
  }
  if (targetsToValidate.includes('mobile')) {
    run('node', [path.join(ROOT, 'check-docker-mobile-config.mjs')], { cwd: ROOT });
  }
}
