...
if (fs.existsSync(webDockerPath)) {
  ...
  mustMatch(t, /FROM\s+node:.*\s+AS\s+build/i, 'web/Dockerfile multi-stage (build)');
  mustMatch(t, /FROM\s+nginx:.*\s+AS\s+runner/i, 'web/Dockerfile multi-stage (runner)');
  mustInclude(t, '/usr/share/nginx/html', 'web/Dockerfile runtime output');
  mustMatch(t, /COPY\s+--from=build/i, 'web/Dockerfile multi-stage copy');
  mustMatch(t, /\bUSER\s+nginx\b/i, 'web/Dockerfile non-root (USER nginx)');
}
