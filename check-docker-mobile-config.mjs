 ...
 * - Dockerfile contains HEALTHCHECK tied to Expo web dev server
...
if (fs.existsSync(appDockerPath)) {
  ...
  mustInclude(t, 'http://127.0.0.1:8081/', 'app/Dockerfile HEALTHCHECK');
  ...
  mustMatch(t, /\bUSER\s+app\b/i, 'app/Dockerfile non-root (USER app)');
  mustMatch(t, /npm\s+run\s+web/i, 'app/Dockerfile expo start');
}
